...
import { borrowApr, revData, revDays } from "@/lib/mock-data";
...
export function RevenueChart() {
  ...
  const x = (i: number) => padL + (i * (W - padL - padR)) / (revData.length - 1);
  ...
  const area = line + `L${x(revData.length - 1).toFixed(1)} ${H - padB} L${x(0).toFixed(1)} ${H - padB} Z`;
  const last = revData.length - 1;
  ...
  return (
    <div className="chart-box" style={{ marginTop: 10 }}>
      <svg
        ref={svgRef}
        viewBox={`0 0 ${W} ${H}`}
        aria-label="Daily revenue, last 30 days, ranging 242 to 439 USDC"
        onMouseMove={onMove}
      ...
      >
        {[250, 350, 450].map((v) => (
          <g key={v}>
            ...
            <text x={padL} y={y(v) - 4} fontSize="9" fill="var(--ink-3)">{v}</text>
          </g>
        ))}
        ...
        {hover && (
          <>
            ...
            <circle cx={x(hover.i)} cy={y(revData[hover.i])} r="4" fill="var(--series-1)" stroke="var(--chart-surface)" strokeWidth="2" />
          </>
        )}
        <circle cx={x(last)} cy={y(revData[last])} r="3.5" fill="var(--series-1)" stroke="var(--chart-surface)" strokeWidth="2" />
        <text x={x(last) - 6} y={y(revData[last]) - 8} fontSize="10" fontWeight="700" fill="var(--ink)" textAnchor="end">
          {revData[last]} USDC
        </text>
      </svg>
      {hover && (
        <div className="tooltip" style={{ left: hover.left, top: hover.top }}>
          <div className="tt-t">{revDays[hover.i]}</div>
          <div className="tt-v mono">{revData[hover.i].toFixed(0)} USDC</div>
        </div>
      )}
    </div>
  );
...
export function RateChart() {
  ...
  const cu = 0.713, ca = borrowApr(cu);
  ...
  return (
    <div className="chart-box">
      <svg
        ref={svgRef}
        viewBox={`0 0 ${W} ${H}`}
        aria-label="Borrow APR versus utilization; 2% at zero, 10% at the 80% kink, 19% at full utilization; current utilization 71.3% gives 9.2% APR"
        onMouseMove={onMove}
        onMouseLeave={() => setHover(null)}
      >
        {[0, 5, 10, 15, 20].map((a) => (
        ...
        <text x={xs(cu) + 9} y={ys(ca) - 9} fontSize="11" fontWeight="700" fill="var(--ink)">
          current · 71.3% → 9.2% APR
        </text>
      </svg>
      {hover && (
        <div className="tooltip" style={{ left: hover.left, top: hover.top }}>
